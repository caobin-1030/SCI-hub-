...
function Search(){
  if($('#input').val().indexOf('10.')==0 || $('#input').val().indexOf('https://doi.org/10')==0){
    ...
    $.post('http://calculate.mysci.online:9000/pubmed/searchPubmedArticle/',parmas,function(res){
      console.log(res.res.articleList)
      if(res.res.articleList!=undefined && res.res.articleList.searchData[0].doi!=undefined){
        $('#searchT').append(`
          <table class="table">
            <tbody>
              <tr>
                <td>标题：</td>
                <td>${res.res.articleList.searchData[0].title}</td>
              </tr>
              <tr>
                <td>DOI：</td>
                <td>${res.res.articleList.searchData[0].doi}</td>
              </tr>
              <tr>
                <td>Journal：</td>
                <td>${res.res.articleList.searchData[0].fullName+`(${res.res.articleList.searchData[0].nowIfs})`}</td>
              </tr>
              <tr>
                <td>下载通道1：</td>
                <td id="tongdao"></td>
