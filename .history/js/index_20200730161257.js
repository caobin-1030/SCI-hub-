...
function download(event){
  ...
  if(event.data.dio==null){
    $("#download")[0].disabled=true
    $("#download")[0].style.cursor='not-allowed'
  }else{
  ...
  if($('#input').val().indexOf('10.')==0 || $('#input').val().indexOf('https://doi.org/10')==0){
    ...
    $.post(`http://calculate.mysci.online:9000/pubmed/searchPubmedArticle/`,params,function(res){
      if(JSON.parse(res).res.articleList!=undefined){
          ...
          <table class="table">
            <tbody>
              ...
              <tr>
                <td>下载通道2：</td>
                <td id="tongdao2">
                  <button id='download' style="${JSON.parse(res).res.articleList.searchData[0].doi==undefined?'cursor:not-allowed':''}" class="btn-sm btn-success">点击下载</button>
                </td>
              </tr>
            </tbody>
          </table>
        ...
        $('#wei').hide()
        $('#searchT').show()
        console.log(JSON.parse(res).res.articleList.searchData[0].doi)
